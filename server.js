// v9 - valida total com QTDE×PRECO + detecta resumo total
...
function parseLines(lines) {
  ...
  let resumoTotal = null; // total oficial do PDF (linha TOTAL GERAL)
  const flagradas = []; // linhas onde QTDE×PRECO difere muito de nums[2]
  ...
  for (const line of lines) {
    ...
    // Captura total do RESUMO (linha que contém "TOTAL GERAL" ou começa com "0061")
    if (resumoTotal === null) {
      const lup = t.toUpperCase();
      if (lup.includes('TOTAL GERAL') || (lup.startsWith('0061') && lup.includes('VINICIUS'))) {
        const nums = t.split(/\s+/).map(parseBR).filter(n => n !== null && n > 10000);
        if (nums.length > 0) {
          resumoTotal = Math.max(...nums);
          console.log('RESUMO TOTAL DETECTADO:', resumoTotal, 'linha:', t.substring(0, 80));
        }
      }
    }
    ...
    const numsRaw = parts.slice(ufIdx + 1);
    const nums = numsRaw.map(parseBR).filter(n => n !== null);
    if (nums.length < 3) continue;

    const total = nums[2];
    if (total <= 0) continue;

    // Validação: QTDE × PRECO deve ≈ total (±20% para cobrir descontos)
    const expectedTotal = nums[0] * nums[1];
    if (expectedTotal > 0.5) {
      const ratio = total / expectedTotal;
      if (ratio > 2.0 || ratio < 0.5) {
        // Total muito diferente do esperado — linha suspeita
        flagradas.push({ pedido: pedidoNum, cod, total, expectedTotal, ratio: ratio.toFixed(2), raw: t.substring(0, 100) });
  ...
  if (flagradas.length > 0) {
    console.log('LINHAS FLAGRADAS (ratio fora de 0.5-2.0):', JSON.stringify(flagradas));
  }
  ...
  return { mes: mesKey || 'jan', label, total: grandTotal, pedidos, meta: 0, clientes, flagradas, resumoTotal };
}
...
app.listen(PORT, () => console.log(`Painel Douratubos v9 rodando em http://localhost:${PORT}`));
