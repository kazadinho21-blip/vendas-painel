// v7 - log linhas suspeitas (total > 5000) para debug
...
function parseLines(lines) {
  ...
  const suspeitas = []; // linhas com total > 5000 para debug
  ...
  for (const line of lines) {
    ...
    // Log linhas com total > 5000 para identificar as 2 extras
    if (total > 5000) {
      suspeitas.push({ pedido: pedidoNum, cod, nome, total, raw: t.substring(0, 120) });
    }
  }

  if (suspeitas.length > 0) {
    console.log('=== LINHAS SUSPEITAS (total > 5000) ===');
    suspeitas.forEach(s => console.log(JSON.stringify(s)));
    console.log('=== FIM ===');
  }
  ...
  return { mes: mesKey || 'jan', label, total: grandTotal, pedidos, meta: 0, clientes, suspeitas };
}
...
app.listen(PORT, () => console.log(`Painel Douratubos v7 rodando em http://localhost:${PORT}`));
