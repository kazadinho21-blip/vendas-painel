// v5 - debug sempre ativo (primeiras 30 linhas capturadas)
...
function parseLines(lines) {
  ...
  for (const line of lines) {
    ...
    // Captura primeiras 30 linhas para debug
    if (debugLines.length < 30) {
      debugLines.push({
        raw: t.substring(0, 150),
        cod,
        uf: parts[ufIdx],
        numsAfterUF: numsRaw.slice(0, 8),
        nums: nums.slice(0, 8),
        totalUsed: total
      });
    }
...
app.listen(PORT, () => console.log(`Painel Douratubos v5 rodando em http://localhost:${PORT}`));
