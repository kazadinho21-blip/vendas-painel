// v4 - debug mode para identificar linhas erradas
...
function parseLines(lines, debug) {
  const clientes = {};
  let pedidos = 0;
  ...
  for (const line of lines) {
    ...
    if (debug) {
      debugLines.push({
        raw: t.substring(0, 120),
        cod, uf: parts[ufIdx],
        numsAfterUF: numsRaw.slice(0, 6),
        nums: nums.slice(0, 6),
        totalUsed: total
      });
    }
  ...
  return { mes: mesKey || 'jan', label, total: grandTotal, pedidos, meta: 0, clientes, debugLines };
}

app.post('/api/parse-text', (req, res) => {
  res.setHeader('Content-Type', 'application/json');
  const { lines, debug } = req.body || {};
  if (!lines || !Array.isArray(lines)) return res.status(400).json({ error: 'lines[] obrigatorio.' });
  try {
    const result = parseLines(lines, !!debug);
...
app.listen(PORT, () => console.log(`Painel Douratubos v4 rodando em http://localhost:${PORT}`));
