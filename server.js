// v8 - filtro DEV + dedup por pedido + allPedidos para debug
...
function parseLines(lines) {
  ...
  for (const line of lines) {
    ...
    // Dedup: se pedido já foi visto, registra como duplicata e ignora
    if (pedidosSeen.has(pedidoNum)) {
      duplicatas.push({ pedido: pedidoNum, cod, total, raw: t.substring(0, 80) });
      continue;
    }
    pedidosSeen.add(pedidoNum);
    pedidos++;
  ...
  if (duplicatas.length > 0) {
    console.log('DUPLICATAS REMOVIDAS:', JSON.stringify(duplicatas));
  }
  ...
  return { mes: mesKey || 'jan', label, total: grandTotal, pedidos, meta: 0, clientes, duplicatas };
}
...
app.listen(PORT, () => console.log(`Painel Douratubos v8 rodando em http://localhost:${PORT}`));
