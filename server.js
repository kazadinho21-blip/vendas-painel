...
// =====================================================
// PARSER v12 — aceita qualquer CFOP, inclui devoluções
// =====================================================
...
function parseLines(lines) {
  ...
  for (const rawLine of lines) {
    ...
    // Detect month/year from period header
    if (!mesKey) {
      const m = line.match(/Per[ií]odo\s+\d{2}\/(\d{2})\/(\d{4})/i) ||
                line.match(/\d{2}\/(\d{2})\/(\d{4})\s+a\s+\d{2}\/\d{2}\/\d{4}/);
      if (m) {
        mesKey = MESES_MAP[parseInt(m[m.length-2])];
        ano = m[m.length-1];
      }
    }

    // Detect declared grand total
    ...
    // Transaction line: starts with DD/MM/YYYY + space
    if (!/^\d{2}\/\d{2}\/\d{4}\s/.test(line)) continue;

    // Must end with 6 numeric columns (supports negatives for devolutions)
    const mV = line.match(/([-\d.]+,\d+)\s+([-\d.]+,\d+)\s+([-\d.]+,\d+)\s+[-\d.]+,\d+\s+[-\d.]+,\d+\s+[-\d.]+,\d+\s*$/);
    if (!mV) continue;

    // Find 5-digit client code before CPF/CNPJ
  ...
  // Remove clients with net zero (fully returned)
  ...
  // Scale to declared total if diff > R$1
...
app.post('/api/parse-text', authMw, (req, res) => {
  ...
  // Allow gerente/admin to save for another user via targetUid
  let saveUid = req.user.id;
  if (targetUid && targetUid !== req.user.id) {
    if (req.user.role !== 'admin' && req.user.role !== 'gerente') {
      return res.status(403).json({ error: 'Apenas gerente/admin pode importar para outro usuario' });
    }
    if (!users[targetUid]) return res.status(404).json({ error: `Usuario "${targetUid}" nao encontrado` });
    saveUid = targetUid;
  }
  ...
  try {
    ...
    if (!salesData[saveUid]) salesData[saveUid] = {};
    salesData[saveUid][result.mes] = result;
    saveSalesFile(saveUid, result.mes, result);
