// v10 - escala via resumoTotal + detecta pedidos do resumo
...
function parseLines(lines) {
  ...
  for (const line of lines) {
    ...
    // Detecta linha TOTAL GERAL ou linha do vendedor (0061 VINICIUS)
    if (resumoTotal === null) {
      ...
      if (lup.includes('TOTAL GERAL') || (lup.startsWith('0061') && lup.includes('VINICIUS'))) {
        ...
        // Busca total: maior número > 100000
        const totais = nums.filter(n => n > 100000);
        if (totais.length > 0) {
          resumoTotal = Math.max(...totais);
          // Busca contagem de pedidos: número inteiro entre 100 e 10000
          const countCandidates = nums.filter(n => n > 100 && n < 10000 && Number.isInteger(n));
          if (countCandidates.length > 0) resumoPedidos = countCandidates[0];
          console.log('RESUMO:', resumoTotal, 'pedidos:', resumoPedidos, '| linha:', t.substring(0, 80));
        }
      }
    }
  ...
  // Aplica scaling se resumoTotal foi detectado e difere do parseado
  if (resumoTotal && resumoTotal > 0 && Math.abs(grandTotal - resumoTotal) > 1) {
    const scale = resumoTotal / grandTotal;
    console.log(`SCALING: ${grandTotal} -> ${resumoTotal} (fator: ${scale.toFixed(6)})`);
  ...
  const finalPedidos = resumoPedidos || pedidos;
  const label = mesKey ? `${MESES_NOME[mesKey]}/${ano}` : 'Desconhecido';

  return { mes: mesKey || 'jan', label, total: grandTotal, pedidos: finalPedidos, meta: 0, clientes, _debug: { parsedTotal: Math.round(Object.values(clientes).reduce((s,c)=>s+c.total,0)*100)/100, parsedPedidos: pedidos, resumoTotal, resumoPedidos } };
}
...
app.listen(PORT, () => console.log(`Painel Douratubos v10 rodando em http://localhost:${PORT}`));
