// v3 - dedup por pedido (multi-item fix)
...
function parseLines(lines) {
  ...
  const pedidosSeen = new Set(); // dedup: evita contar mesmo pedido 2x (multi-item)
  ...
  for (const line of lines) {
    ...
    // Número do pedido (parts[1], ~7 dígitos)
    const pedidoNum = parts[1];
    if (!pedidoNum || !/^\d{5,8}$/.test(pedidoNum)) continue;
    ...
    // Números após UF: QTDE, PRECO_UNIT, TOTAL, ...
    const nums = parts.slice(ufIdx + 1).map(parseBR).filter(n => n !== null);
    if (nums.length < 3) continue;

    const total = nums[2]; // coluna TOTAL
    if (total <= 0) continue;

    // Ignorar linhas de pedidos já contados (itens extras do mesmo pedido)
    if (pedidosSeen.has(pedidoNum)) continue;
...
app.listen(PORT, () => {
  console.log(`Painel Douratubos v3 rodando em http://localhost:${PORT}`);
});
